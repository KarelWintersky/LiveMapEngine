...
class JUpload
{
    ...
    public function __construct($appletparams = array(), $classparams = array())
    {
        ...
        if ($classparams[ 'create_destdir' ]) {
            ...
            if (!mkdir( $concurrentDirectory = $classparams[ 'destdir' ], $classparams[ 'dirperm' ] ) && !is_dir( $concurrentDirectory )) {
     ...
     *
     * @return string, containing the necessary wrapper function (named JUploadSetProperty)
     */
     ...
     *
     * @return string, containing the applet tag
     */
    ...
    private function mkdirp($path)
    {
        ...
        foreach ($dirs as $dir) {
            ...
            if (!file_exists( $path )) {  // @ does NOT always supress the error!
                $_umask = umask( 0 );    // override the system mask
                if (!mkdir( $path, $this->classparams[ 'dirperm' ] ) && !is_dir( $path )) {
