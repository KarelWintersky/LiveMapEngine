...
function create_img_gd($imgfile, $imgthumb, $newwidth, $newheight = "")
{
    if (image_check_memory_usage($imgfile, $newwidth, $newheight)) {
        ...
        $magicianObj = new imageLib($imgfile); // файл существует, путь корректный
...
https://github.com/trippo/ResponsiveFilemanager/issues/362
function image_check_memory_usage($img, $max_breedte, $max_hoogte)
{
    if (file_exists($img)) {
        ...
        $memory_limit_unit = strtoupper(substr($memory_limit, -1));
        $memory_limit = abs(match ($memory_limit_unit) {
            'G' => (int)$memory_limit * 1024 ** 3,
            'M' => (int)$memory_limit * 1024 ** 2,
            'K' => (int)$memory_limit * 1024,
            default => (int)$memory_limit,
        });
